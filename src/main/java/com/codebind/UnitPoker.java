...
public class UnitPoker {
	...
	public ArrayList<Object[]> nextUnit(Player p1, Player p2, int[][] share, int[][] hand1, int[][] hand2, Deck deck,
			...
			ArrayList<float[][]> histData) throws CloneNotSupportedException {
		int[][] totHand1 = new int[hand1.length + share.length][];
		int[][] totHand2 = new int[hand1.length + share.length][];
		...
		for (int i = 0; i < plays.length; i++) {
			...
			if (cont) {
				if (!dealCards)
					out.add(new Object[] { p1.getCopy(), p2.getCopy(), share, hand1, hand2, deck, pot, minbet, callable,
							!flip, round, raisItr, dataFile, histData });
				else {
					Deck tempDeck = ((Deck) deck).getCopy();
					int[][] newCards = dealCards(tempDeck, round - 1);
					int[][] totHand = new int[newCards.length + share.length + hand1.length][];
					for (int k = 0; k < share.length; k++) {
						totHand[k] = share[k];
					}
					for (int k = 0; k < newCards.length; k++) {
						totHand[k + share.length] = newCards[k];
					}
					for (int k = 0; k < hand1.length; k++) {
						totHand[k + share.length + newCards.length] = hand1[k];
					}
	...
	public Object[] playHand(Player p1, Player p2, boolean flip, int dataFile) throws CloneNotSupportedException {
		Deck deck = new Deck();
		int round = 0;
		double pot = 0;
		if (flip) {
